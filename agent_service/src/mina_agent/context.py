...
BASE_SYSTEM_SECTIONS = (
    ...
    (
        ...
        "5. For coordinate math, call coordinate_math. For exact common Minecraft recipe/item facts, call recipe_lookup/item_lookup. For Minecraft wiki/current mechanics/farms/redstone/tutorials, prefer minecraft_wiki_search. For current non-Minecraft knowledge, web/search wording, outside verification, or corrections, call web_search. Use web_fetch for a specific URL or insufficient snippets. Do not rely on older conversation for current farm/redstone/tutorial facts.\n"
    ...
    (
        ...
        "- From web_search, minecraft_wiki_search, or web_fetch results, preserve exact source values such as markers, versions, coordinates, URLs, and item names. Do not replace exact values with generic labels.\n"
        "- If web/wiki evidence_quality is low/none, or results have low_relevance/missing_query_terms for the requested build/mechanic, say the search evidence is not specific enough and ask for a link/project/screenshot or permission to search a narrower query. Do not invent exact redstone/farm steps from weak snippets.\n"
...
def _render_history_compatibility_warning(history: list[dict[str, Any]]) -> str:
    ...
    return "\n".join(
        [
            ...
            "- Current capabilities are text conversation, web/wiki knowledge, memory, observed Minecraft state, coordinate math, common item/recipe lookup, and tightly constrained read-only Minecraft commands.",
            "- Older assistant answers about Minecraft mechanics can be stale or wrong; use minecraft_wiki_search for current farm/redstone/tutorial advice.",
        ]
    )
