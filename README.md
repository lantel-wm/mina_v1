...
The E2E runner loads `agent_service/.env`, requires a real DeepSeek `MINA_API_KEY`, refuses loopback/mock DeepSeek endpoints, starts the real sidecar, a deterministic SearXNG-compatible search fixture, and a headless Fabric/PuppetPlayers server, drives declarative `/mina-test` scenarios, and writes trace artifacts plus model token totals under `build/e2e/runs/`. Pass `--searxng-url` to point the run at an external SearXNG instance instead. The sidecar exposes `/v1/model-calls`, `/v1/tool-calls`, `/v1/action-events`, `/v1/tasks`, and `/v1/traces/{request_id}` for debugging failed runs.
